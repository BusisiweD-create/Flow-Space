                  ...
                  child: DropdownButton<String>(
                    ...
                    onChanged: (String? newStatus) {
                      if (newStatus != null && newStatus != sprint['status']) {
                        _updateSprintStatus(sprint['id'].toString(), newStatus);
                      }
                    },
                  ),
                ),
  ...
  Future<void> _createSprint(String name, String description, String startDate, String endDate) async {
    try {
      ...
      if (result != null) {
        ...
        try {
          setState(() {
            _sprints = [result, ..._sprints];
            _selectedSprintId = result['id']?.toString();
          });
                    ...
                    children: [
                      Expanded(
                        child: SwitchListTile(
                          value: useAi,
                          onChanged: (v) {
                            dialogSetState(() { useAi = v; });
                            setState(() { _useAiForTicket = v; });
                          },
                  ...
                  try {
                    if (useAi) {
                      ...
                      try {
                        ...
                        final resp = await backend.aiChat(messages, temperature: 0.4, maxTokens: 256);
                        if (resp.isSuccess && resp.data != null && (titleController.text.isEmpty || descriptionController.text.isEmpty)) {
                          final data = resp.data is Map ? Map<String, dynamic>.from(resp.data as Map) : {};
                          final content = (data['content'] ?? (data['data']?['content']))?.toString() ?? '';
                          String t = '';
                          String d = '';
