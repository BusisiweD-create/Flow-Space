    ...
    if (userId && userText) {
      ...
      if (pending) {
        ...
        if (pending.type === 'signoff_sprint_report') {
          ...
          if (pending.data.report_title && !isConfirmText(userText)) {
            return sendResponse(res, true, {
              content: cleanAiText(`Report title set to: ${String(pending.data.report_title)}\nReply "confirm" to create and export the sign-off report PDF, or "cancel".`),
              usage: {},
              model: 'server',
            });
          }
          ...
          if (!isConfirmText(userText)) {
            return sendResponse(res, true, { content: cleanAiText('Reply "confirm" to create and export the sign-off report PDF, or "cancel".'), usage: {}, model: 'server' });
          }
